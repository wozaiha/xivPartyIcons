...
public sealed class RoleTracker : IDisposable
{
    ...
    {
        Service.Log.Verbose("Resetting occupation");
        _occupiedRoles.Clear();
    }

    public void ResetAssignments()
    {
        Service.Log.Verbose("Resetting assignments");
    ...
    public void CalculateUnassignedPartyRoles()
    {
        ...
        Service.Log.Verbose($"Assigning current occupations ({_occupiedRoles.Count})");

        foreach (var kv in _occupiedRoles)
        {
            Service.Log.Verbose($"{kv.Key} == {kv.Value} as per occupation");

            _assignedRoles[kv.Key] = kv.Value;
            _unassignedRoles.Remove(kv.Value);
        }

        Service.Log.Verbose($"Assigning static assignments ({_configuration.StaticAssignments.Count})");
        ...
        foreach (var kv in _configuration.StaticAssignments)
        {
            foreach (var member in Service.PartyList)
            {
                ...
                {
                    Service.Log.Verbose($"{PlayerId(member)} has already been assigned a role");
                ...
                if (kv.Key.Equals(playerDescription))
                {
                    ...
                    {
                        Service.Log.Verbose($"{playerId} == {kv.Value} as per static assignments {playerDescription}");
        ...
        Service.Log.Verbose("Assigning the rest");
        ...
        foreach (var member in Service.PartyList)
        {
            ...
            {
                Service.Log.Verbose($"{PlayerId(member)} == {roleToAssign} as per first available");
    ...
    private void FrameworkOnUpdate(IFramework framework)
    {
        if (!Service.Condition[ConditionFlag.ParticipatingInCrossWorldPartyOrAlliance]
            ...
            _occupiedRoles.Any())
        {
            Service.Log.Verbose("Resetting occupations, no longer in a party");
            ResetOccupations();
        ...
        {
            Service.Log.Verbose($"Party hash changed ({partyHash}, prev {_previousStateHash}), recalculating roles");
        ...
        ref bool ishandled)
    {
        if (_configuration.AssignFromChat && (type == XivChatType.Party || type == XivChatType.CrossParty || type == XivChatType.Say))
        {
            ...
            {
                Service.Log.Verbose($"Failed to get player data from {senderid}, {sender} ({sender.Payloads})");
            ...
            foreach (var tuple in _occupationMessages)
            {
                if (tuple.Item2.Equals(paddedText))
                {
                    ...
                    {
                        Service.Log.Verbose($"Multiple role occupation matches, aborting");
            ...
            foreach (var tuple in _suggestionRegexes)
            {
                if (tuple.Item2.IsMatch(paddedText))
                {
                    ...
                    {
                        Service.Log.Verbose("Multiple role suggesting matches, aborting");
            ...
            if (!occupationTainted && roleToOccupy != RoleId.Undefined)
            {
                ...
                Service.Log.Verbose($"Recalculating assignments due to new occupations");
                CalculateUnassignedPartyRoles();
            }
